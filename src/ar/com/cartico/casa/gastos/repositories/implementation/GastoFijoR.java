...
public class GastoFijoR implements I_GastoRepository{
    private Connection conn;
    public GastoFijoR (Connection conn){ this.conn = conn;}
    
    @Override
    public void save(gasto gasto) {
        if(gasto==null)return;
        try (PreparedStatement ps = conn.prepareStatement(
              "insert into gasto_fijo (año, mes, detalle, idPersona, monto)"
    ...
    public void remove(gasto gasto) {
        if(gasto==null)return;
        try (PreparedStatement ps = conn.prepareStatement("delete from gasto_fijo where id=?")){
    ...
    public void update(gasto gasto) {
        if(gasto==null)return;
        try (PreparedStatement ps = conn.prepareStatement(
                "update gasto_fijo set año=?, mes=?, detalle=?, idpersona=?, monto=? where id=?"
        )){ 
            ps.setInt(1, gasto.getAño());
            ps.setString(2, gasto.getMes().toString());
            ps.setString(3, gasto.getDetalle());
    ...
    */
    private List<gasto> getByFiltro(String filtro){
        ...
        String query = "Select * from gasto_fijo where " + filtro;
    ...
    */
    private int getTotalByFiltro (String filtro){
        int total = 0;
        String query = "select monto from gasto_fijo where " + filtro;
        try (ResultSet rs = conn.createStatement().executeQuery(query)){
            while(rs.next()){
    ...
    public List<String> getDetalles() {
        ...
        String sql="Select detalle from gasto_fijo group by detalle";
    ...
    public List<String> getAños() {
         List<String> lista = new ArrayList();
        String sql="Select año from gasto_fijo group by año";
        try (ResultSet rs = conn.createStatement().executeQuery(sql)){
            while(rs.next()){
    ...
    public List<gasto> getDeuda() {
        ...
        int cantidadPersonas = pr.totalImplicanciaGF();
        int total = getTotal();
        List<gasto> lista = new ArrayList();
        String sql = "Select idpersona, sum(monto) as total from gasto_fijo group by idpersona";
        try (ResultSet rs = conn.createStatement().executeQuery(sql)){
            while(rs.next()){
                int idPersona = rs.getInt("idpersona");
                int monto = ((total/cantidadPersonas)*pr.getById(idPersona).getImplicanciaGF())-rs.getInt("total");
                lista.add(new gasto(
                        idPersona,
                        monto
                ));
